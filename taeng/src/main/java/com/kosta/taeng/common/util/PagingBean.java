...
/**
 * 페이징 처리위한 bean <br>
 * 항목이 너무 많아 한페이지에서 보여주기 힘들때 특정 개수만끔 나누어 보여준다. 이때 필요한 요소들을 계산해서 전달해주는 기능을 제공하는
 * 클래스.
 * <p>
 * 용어 item : 하나의 데이터 page : 데이터들의 묶음 page group : page들의 묶음
 * 
 * @author kgmyh
 *
 */
public class PagingBean {
	...
	private int getTotalPageGroup() {
		...
		if (totalPage % pagesPerPagegroup != 0) {
			return totalPage / pagesPerPagegroup + 1;
		} else {
			return totalPage / pagesPerPagegroup;
		}
	}
	...
	private int getCurrentPageGroup() {
		...
		if (page % pagesPerPagegroup != 0) {
			return page / pagesPerPagegroup + 1;
		} else {
			return page / pagesPerPagegroup;
		}
	}

	/**
	 * 현재 페이지(this.page) 에 보여질 시작 item의 번호(rownum번호) 조회 마지막 item번호 - 페이지당 보여질 item수
	 * + 1
	 * 
	 * @return
	 */
	...
	/**
	 * 현재 페이지(this.page) 에 보여질 마지막 item의 번호(rownum번호) 조회 현재페이지(this.page) * 페이지동 보여질
	 * item수
	 * 
	 * @return
	 */
	 ...
	 * 1. Page Group 내 Page 수*(현재 페이지 그룹 -1) + 1을 한 값이 첫 페이지이다.(페이지 그룹*페이지 그룹 개수 이 그
	 * 그룹의 마지막 번호이므로)<br>
